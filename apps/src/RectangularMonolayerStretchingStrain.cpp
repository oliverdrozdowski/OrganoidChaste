...
struct paramsMonolayerStretchStrainX
{
    double GammaA = 1.0;
    double GammaB = 1.0;
    int NumCellsY = 20;
...
paramsMonolayerStretchStrainX ParseArguments(int argc, char* argv[])
{
    paramsMonolayerStretchStrainX params;
    ...
    for (int i = 1; i < argc; i++)
    {
        ...
        if (arg == "-h")
        {
            ...
            std::cout << "  -StrainX <value>   : StrainX variable for given tensor (std: 0.0)" << std::endl;
            std::cout << "  -StrainY <value>   : StrainY variable for given tensor (std: 0.0)" << std::endl;
        ...
        else if (arg == "-StrainX" && i + 1 < argc)
        {
            params.StrainX = std::stod(argv[i + 1]);
        }
        else if (arg == "-StrainY" && i + 1 < argc)
        {
            params.StrainY = std::stod(argv[i + 1]);
        }
 ...
 */
std::string CreateFilePath(const paramsMonolayerStretchStrainX& params)
{
    ...
    filepath << "Strain_Stretched_Honeycomb_"
             << "GA" << params.GammaA << "_"
             << "GB" << params.GammaB << "_"
             << "NX" << params.NumCellsX << "_"
             << "NY" << params.NumCellsY << "_"
             << "EPX" << params.StrainX << "_"
             << "EPY" << params.StrainY << "_"
...
void SaveConstantsToFile(const std::string& filepath, const paramsMonolayerStretchStrainX& params,
                         double numNodesX, double numNodesY, double numNodesTop, double numNodesBottom)
{
    ...
    if (output_file->is_open())
    {
        ...
        *output_file << "StrainX = " << params.StrainX << std::endl;
        *output_file << "StrainY = " << params.StrainY << std::endl;
...
int main(int argc, char* argv[])
{
    ...
    try
    {
        ...
        paramsMonolayerStretchStrainX simulation_parameters = ParseArguments(argc, argv);
        ...
        */
        ...
        double force_xx = 1.0 * simulation_parameters.StrainX / num_right_nodes;
        double force_yy = 1.0 * simulation_parameters.StrainY / num_top_nodes;
        ...
        // Scale the nodes' positions according to the strain from boundary forces
        // double youngs_modulus = 2.0 * (gamma_a + gamma_b);
        double poisson_ratio = 1.0 / 3.0;
        // double s_edge = 1.0 / cbrt(3.0 * 3.0 * (gamma_a + gamma_b) / 2.0);
        // double s_x = sqrt(3) * s_edge / 2.0;
        // double l_x = (num_cells_x)*sqrt(3) * s_edge;
        // double l_y = (num_cells_y)*1.5 * s_edge;

        double strain_xx = simulation_parameters.StrainX;
        double strain_yy = -poisson_ratio * strain_xx;

        unsigned num_nodes = p_mesh->GetNumNodes();
        std::vector<Node<3>*> all_nodes;
        for (unsigned index_node = 0; index_node < num_nodes; ++index_node)
        {
            Node<3>* p_node = p_mesh->GetNode(index_node);
            all_nodes.push_back(p_node);
            c_vector<double, 3>& r_node_vector = p_node->rGetModifiableLocation();
            r_node_vector[0] *= (1.0 + strain_xx);
            r_node_vector[1] *= (1.0 + strain_yy);
        }
        ...
        MAKE_PTR_ARGS(FixNodeDimensionsBoundaryCondition<3>,
                      p_left_boundary_bc,
                      (&cell_population, all_nodes, true, false, false));
        simulator.AddCellPopulationBoundaryCondition(p_left_boundary_bc);

        /*
        MAKE_PTR_ARGS(FixNodeDimensionsBoundaryCondition<3>,
                      p_right_boundary_bc,
                      (&cell_population, right_edge_nodes, true, false, false));
        simulator.AddCellPopulationBoundaryCondition(p_right_boundary_bc);
        */
