...
std::string CreateFilePath(const paramsMonolayerStretch& params)
{
    ...
    filepath << "Stretched_Flattened_Stress_Honeycomb_"
             << "GA" << params.GammaA << "_"
             << "GB" << params.GammaB << "_"
...
int main(int argc, char* argv[])
{
    ...
    try
    {
        ...
        */
        ...
        std::vector<Node<3>*> node_vector_upper_plane;
        std::vector<Node<3>*> node_vector_lower_plane;
        unsigned num_nodes = p_mesh->GetNumNodes();
        for (unsigned index_node = 0; index_node < num_nodes; ++index_node)
        {
            if (p_mesh->GetNode(index_node)->rGetLocation()[2] > height / 2.0)
            {
                node_vector_upper_plane.push_back(p_mesh->GetNode(index_node));
            }
            else if (p_mesh->GetNode(index_node)->rGetLocation()[2] <= height / 2.0)
            {
                node_vector_lower_plane.push_back(p_mesh->GetNode(index_node));
            }
        }

        c_vector<double, 3> plane_normal_tb = zero_vector<double>(3);
        plane_normal_tb[2] = 1.0;

        MAKE_PTR_ARGS(NodesFlattenedPlaneBoundaryCondition<3>,
                      p_flat_top_bc,
                      (&cell_population, plane_normal_tb));
        p_flat_top_bc->SetEdgeNodes(node_vector_upper_plane);

        MAKE_PTR_ARGS(NodesFlattenedPlaneBoundaryCondition<3>,
                      p_flat_bottom_bc,
                      (&cell_population, plane_normal_tb));
        p_flat_bottom_bc->SetEdgeNodes(node_vector_lower_plane);

        simulator.AddCellPopulationBoundaryCondition(p_flat_top_bc);
        simulator.AddCellPopulationBoundaryCondition(p_flat_bottom_bc);
