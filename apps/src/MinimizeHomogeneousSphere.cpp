...
struct paramsHomogeneousSphere
{
    unsigned NumCells = 100;
    unsigned PrintEveryNSteps = 2499;
    double MaxSimulationTime = 1e4;
    double MaxTimeLoopStepping = 20000.0;
    double FinalDT = 0.1;
    double AreaDifferenceCutoff = 1e-4;
    ...
    double ApicalTension = 0.9;
    double BasalTension = 0.9;
    double LateralTension = 1.0;
    std::string FileComment = "";
};

paramsHomogeneousSphere ParseArguments(int argc, char* argv[])
{
    paramsHomogeneousSphere params;
    ...
    for (int i = 1; i < argc; i++)
    {
        ...
        if (arg == "-h")
        {
            ...
            std::cout << "  -ApicalTension <value>   : Apical Tension (std: 0.9)" << std::endl;
            std::cout << "  -BasalTension <value>   : Basal Tension (std: 0.9)" << std::endl;
            std::cout << "  -LateralTension <value>   : Lateral Tension (std: 1.0)" << std::endl;
        ...
        else if (arg == "-ApicalTension" && i + 1 < argc)
        {
            params.ApicalTension = std::stod(argv[i + 1]);
        }
        else if (arg == "-BasalTension" && i + 1 < argc)
        {
            params.BasalTension = std::stod(argv[i + 1]);
        }
        else if (arg == "-LateralTension" && i + 1 < argc)
        {
            params.LateralTension = std::stod(argv[i + 1]);
        }
 ...
 */
std::string CreateFilePath(const paramsHomogeneousSphere& params)
{
    ...
    filepath << "RandomizedHomogeneousSphere_"
             << "NUM" << params.NumCells << "_"
             << "GAP" << params.ApicalTension << "_"
             << "GBA" << params.BasalTension << "_"
             << "GLA" << params.LateralTension << "_"
...
void SaveConstantsToFile(const std::string& filepath, const paramsHomogeneousSphere& params)
{
    OutputFileHandler handler(filepath, false);
    out_stream output_file = handler.OpenOutputFile("MosaicSphereParameters.txt");

    if (output_file->is_open())
    {
        *output_file << "NumCells = " << params.NumCells << std::endl;
        *output_file << "ApicalTension = " << params.ApicalTension << std::endl;
        *output_file << "BasalTension = " << params.BasalTension << std::endl;
        *output_file << "LateralTension = " << params.LateralTension << std::endl;
...
int main(int argc, char* argv[])
{
    ...
    try
    {
        ...
        // Parse the input
        paramsHomogeneousSphere simulation_parameters = ParseArguments(argc, argv);
        ...
        double mean_apical = simulation_parameters.ApicalTension;
        double mean_basal = simulation_parameters.BasalTension;
        double mean_lateral = simulation_parameters.LateralTension;

        // alpha = apical/lateral surface tension
        // beta = basal/lateral surface tension
        double alpha_avg = mean_apical / mean_lateral;
        double beta_avg = mean_basal / mean_lateral;
        ...
        std::vector<boost::shared_ptr<MechanicalMosaicCellMutationState> > mutation_states;
        ...
        for (unsigned cell_index = 0; cell_index < num_cells; cell_index++)
        {
            ...
            MAKE_PTR(MechanicalMosaicCellMutationState, p_state);
            mutation_states.push_back(p_state);

            // double tension_a = RandomNumberGenerator::Instance()->NormalRandomDeviate(mean_apical, 0.03);
            // double tension_b = RandomNumberGenerator::Instance()->NormalRandomDeviate(mean_basal, 0.03);
            // double tension_l = RandomNumberGenerator::Instance()->NormalRandomDeviate(mean_lateral, 0.05);
            double tension_a = mean_apical;
            double tension_b = mean_basal;
            double tension_l = mean_lateral;

            // Make a dictionary for mosaic cell surface tension parameters
            std::array<double, 3> mosaic_surface_tensions = { tension_a, tension_b, tension_l }; // apical, basal, lateral
            dictionary_surface_tensions[p_state] = mosaic_surface_tensions;

            p_cell = CellPtr(new Cell(p_state, p_model));
