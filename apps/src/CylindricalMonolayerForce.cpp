...
struct paramsCylindricalMonolayer
{
    double GammaA = 1.0;
    double GammaB = 1.0;
    int NumCellsY = 20;
...
paramsCylindricalMonolayer ParseArguments(int argc, char* argv[])
{
    paramsCylindricalMonolayer params;
 ...
 */
std::string CreateFilePath(const paramsCylindricalMonolayer& params)
{
    ...
    filepath << "Stressed_Cylindrical_Honeycomb_"
             << "GA" << params.GammaA << "_"
             << "GB" << params.GammaB << "_"
...
void SaveConstantsToFile(const std::string& filepath, const paramsCylindricalMonolayer& params,
...
int main(int argc, char* argv[])
{
    ...
    try
    {
        ...
        paramsCylindricalMonolayer simulation_parameters = ParseArguments(argc, argv);
