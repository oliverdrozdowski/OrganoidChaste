...
paramsMonolayer ParseArguments(int argc, char* argv[])
{
    ...
    for (int i = 1; i < argc; i++)
    {
        ...
        if (arg == "-h")
        {
            ...
            std::cout << "  -CenterForceAsymm <value>   : (bool) Whether the center force is applied asymmetrically and not in central cells (std: 0)" << std::endl;
            ...
            std::cout << "  -StraightEdges <value>   : bool whether edges are kept straight (std: 0)" << std::endl;
        ...
        else if (arg == "-CenterForceAsymm" && i + 1 < argc)
        {
            params.AsymmCenterForce = (bool)std::stoi(argv[i + 1]);
        }
        ...
        else if (arg == "-StraightEdges" && i + 1 < argc)
        {
            params.StraightEdges = (bool)std::stoi(argv[i + 1]);
        }
...
std::string CreateFilePath(const paramsMonolayer& params)
{
    ...
    if (params.StraightEdges)
    {
        straight_string = "_StraightEdges";
    }

    std::ostringstream filepath;
    filepath << "Compressed_Stress_Honeycomb_"
             << "GA" << params.GammaA << "_"
             << "GB" << params.GammaB << "_"
                         ...
                         double numNodesX, double numNodesY, double numNodesTop, double numNodesBottom)
{
    ...
    if (output_file->is_open())
    {
        ...
        *output_file << "CenterForceAsymm = " << params.AsymmCenterForce << std::endl;
...
int main(int argc, char* argv[])
{
    ...
    try
    {
        ...
        paramsMonolayer simulation_parameters = ParseArguments(argc, argv);
        ...
        */
        ...
        std::vector<Node<3>*> node_vector_left_constraint;
        std::vector<Node<3>*> node_vector_right_constraint;
        std::vector<Node<3>*> node_vector_top_constraint;
        std::vector<Node<3>*> node_vector_bottom_constraint;
        unsigned num_nodes = p_mesh->GetNumNodes();
        for (unsigned index_node = 0; index_node < num_nodes; ++index_node)
        {
            if (p_mesh->GetNode(index_node)->rGetLocation()[0] < 1e-3)
            {
                node_vector_height_constraint.push_back(p_mesh->GetNode(index_node));
                node_vector_left_constraint.push_back(p_mesh->GetNode(index_node));
            }
            else if (p_mesh->GetNode(index_node)->rGetLocation()[0] > l_x + sqrt(3) * s_edge / 2.0 - 1e-3)
            {
                node_vector_height_constraint.push_back(p_mesh->GetNode(index_node));
                node_vector_right_constraint.push_back(p_mesh->GetNode(index_node));
            }
            if ((p_mesh->GetNode(index_node)->rGetLocation()[1] < 1e-3))
            {
                node_vector_height_constraint.push_back(p_mesh->GetNode(index_node));
                node_vector_bottom_constraint.push_back(p_mesh->GetNode(index_node));
            }
            else if ((p_mesh->GetNode(index_node)->rGetLocation()[1] > l_y + s_edge / 2.0 - 1e-3))
            {
                node_vector_height_constraint.push_back(p_mesh->GetNode(index_node));
                node_vector_top_constraint.push_back(p_mesh->GetNode(index_node));
            }
        }

        c_vector<double, 3> mid_zero_point = zero_vector<double>(3);
        c_vector<double, 3> mid_normal_vector = zero_vector<double>(3);
        c_vector<double, 3> edge_normal_tb = zero_vector<double>(3);
        c_vector<double, 3> edge_normal_lr = zero_vector<double>(3);
        mid_normal_vector[2] = 1.0;
        edge_normal_tb[1] = 1.0;
        edge_normal_lr[0] = 1.0;
        ...
        MAKE_PTR_ARGS(StraightEdgeBoundaryCondition<3>,
                      p_left_edge_straight_bc,
                      (&cell_population, edge_normal_lr));
        p_left_edge_straight_bc->SetEdgeNodes(node_vector_left_constraint);

        MAKE_PTR_ARGS(StraightEdgeBoundaryCondition<3>,
                      p_right_edge_straight_bc,
                      (&cell_population, edge_normal_lr));
        p_right_edge_straight_bc->SetEdgeNodes(node_vector_right_constraint);

        MAKE_PTR_ARGS(StraightEdgeBoundaryCondition<3>,
                      p_top_edge_straight_bc,
                      (&cell_population, edge_normal_tb));
        p_top_edge_straight_bc->SetEdgeNodes(node_vector_top_constraint);

        MAKE_PTR_ARGS(StraightEdgeBoundaryCondition<3>,
                      p_bottom_edge_straight_bc,
                      (&cell_population, edge_normal_tb));
        p_bottom_edge_straight_bc->SetEdgeNodes(node_vector_bottom_constraint);

        if (simulation_parameters.StraightEdges)
        {
            simulator.AddCellPopulationBoundaryCondition(p_left_edge_straight_bc);
            simulator.AddCellPopulationBoundaryCondition(p_right_edge_straight_bc);
            simulator.AddCellPopulationBoundaryCondition(p_top_edge_straight_bc);
            simulator.AddCellPopulationBoundaryCondition(p_bottom_edge_straight_bc);
        }
        ...
        bool asymm_center = simulation_parameters.AsymmCenterForce;
        unsigned center_index = (num_cells_y / 2) * num_cells_x + (num_cells_x / 2);
        unsigned center_index_prev_row = (num_cells_y / 2 - 1) * num_cells_x + (num_cells_x / 2);

        // Correct if asymm
        center_index -= asymm_center ? (unsigned)(num_cells_x / 6.0) : 0;
        center_index_prev_row -= asymm_center ? (unsigned)(num_cells_x / 6.0) : 0;
        ...
        // MonolayerVertexElement<3, 3>* p_element = p_mesh->GetElement(center_index);
