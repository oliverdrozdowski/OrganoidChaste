/**
 * Perform a one-sided compression of a shell centered around the origin. The initial mesh is a MutableMonolayerVertexMesh read from a given
 * directory and which was written with MonolayerVertexMeshWriter. The compression is only done from the top, i.e. only one boundary, and all
 * nodes with z<0 are fixed in their z-postion to "hold" the shell in its place.
 */
...
paramsMonolayer ParseArguments(int argc, char* argv[])
{
    ...
    for (int i = 1; i < argc; i++)
    {
        ...
        else if (arg == "-DoInitialRelax" && i + 1 < argc)
        {
            params.DoInitialRelax = std::stod(argv[i + 1]);
        }
        i++;
    }
...
void SaveConstantsToFile(const std::string& filepath, const paramsMonolayer& params, double numCells)
{
    ...
    if (output_file->is_open())
    {
        ...
        *output_file << "DoInitialRelax = " << params.DoInitialRelax << std::endl;
        output_file->close();
...
int main(int argc, char* argv[])
{
    ...
    try
    {
        ...
        if (parameters.DoInitialRelax)
        {
            ...
            std::cout << "Do initial relax" << std::endl;
            simulator.Solve();

            start_time = 20.0;
        }

        // Add boundary conditions which fixes the lower half of the sphere
        // Identify nodes which are on the lower half
        ...
        {
            if (p_mesh->GetNode(index_node)->rGetLocation()[2] < 0)
            {
        ...
        MAKE_PTR_ARGS(FixNodeDimensionsBoundaryCondition<3>,
                      p_lower_boundary_bc,
                      (&cell_population, node_vector_bottom, true, true, true));
        ...
        double time_after_step = time_for_one_step + start_time;

        MAKE_PTR_ARGS(OneStepProfile<3>, linear_move_profile_top, (start_time, time_after_step, starting_position, (starting_position - distance_per_step)));
