...
std::string CreateFilePath(const paramsMosaicSphere& params)
{
    ...
    filepath << "RandomizedNormalMosaicSphereBuddedPatterned_"
...
int main(int argc, char* argv[])
{
    ...
    try
    {
        ...
        /*
        // Parameters optimized for 600 cells
        double mean_apical = 1.035;
        double mean_basal = 0.965;
        double mean_lateral = 1.0;

        double mean_apical_bud = 1.08;
        double mean_basal_bud = 0.92;
        double mean_lateral_bud = 1.0;

        double mean_apical_neck = 0.98;
        double mean_basal_neck = 1.02;
        double mean_lateral_neck = 1.0;
        */

        double mean_apical = 1.15;
        double mean_basal = 0.95;
        double mean_lateral = 1.0;

        double mean_apical_bud = 1.30;
        double mean_basal_bud = 0.95;
        double mean_lateral_bud = 1.0;

        double mean_apical_neck = 1.0;
        double mean_basal_neck = 1.0;
        double mean_lateral_neck = 1.0;
        ...
        {
            c_vector<double, 3> cell_center = p_mesh->GetCentroidOfElement(cell_index);
            double apical_tension = 0.0;
            double basal_tension = 0.0;
            double lateral_tension = 0.0;

            if (cell_center[2] >= 4.0 * radius / 5.0)
            {
                apical_tension = mean_apical_bud;
                basal_tension = mean_basal_bud;
                lateral_tension = mean_lateral_bud;
            }
            else if (cell_center[2] >= 2.5 * radius / 5.0)
            {
                apical_tension = mean_apical_neck;
                basal_tension = mean_basal_neck;
                lateral_tension = mean_lateral_neck;
            }
            else
            {
                apical_tension = mean_apical;
                basal_tension = mean_basal;
                lateral_tension = mean_lateral;
            }
            ...
            double tension_a = RandomNumberGenerator::Instance()->NormalRandomDeviate(apical_tension, 0.015);
            double tension_b = RandomNumberGenerator::Instance()->NormalRandomDeviate(basal_tension, 0.015);
            double tension_l = RandomNumberGenerator::Instance()->NormalRandomDeviate(lateral_tension, 0.015);
        ...
        // cell_population.AddFaceWriter<FaceTensionWriter>();
        ...
        p_random_force->SetDecayTimeParameter(1500.0);
        p_random_force->SetRelativeStrengthParameter(0.0);
        ...
        // p_tension_subforce->SetSimulatedAnnealingParameters(0.1, 50.0, 0.0); // zero temp anyways
        ...
        p_activeT1_modifier->SetActiveT1Rate(4.0);
        simulator.AddSimulationModifier(p_activeT1_modifier);

        MAKE_PTR(Temperature, temperature);
        temperature->SetStartTime(0.0);
        temperature->SetAbsoluteDecayTime(1000.0);

        // Make T-dependent T1 rate (modifier)
        p_activeT1_modifier->SetTemperature(temperature);

        // p_tension_subforce->SetSimulatedAnnealingParameters(0.1, 1000.0, 4.0);
        p_random_force->SetRelativeStrengthParameter(0.03);
