...
class TestIcosahedralSphereParsed : public AbstractCellBasedTestSuite
{
private:
    unsigned mCasparKlugH = 1;
    unsigned mCasparKlugK = 0;
    double mAlpha = 1.0;
    double mBeta = 1.0;
    std::string mSavePath = "Test3DSphere_icosahedral_CK_1_0_Gamma_1.0_init_sphere";

    void ParseParameters()
    {
        // Determine the Config filepath
        FileFinder reference("projects/OrganoidChaste/test/sing_simulations/IcosahedralSphereParsedConfig.txt",
        ...
        if (config_file.is_open())
        {
            ...
            while (getline(config_file, line))
            {
                ...
                if (name == "Caspar_Klug_H")
                    mCasparKlugH = atoi(value.c_str());
                else if (name == "Caspar_Klug_K")
                    mCasparKlugK = atoi(value.c_str());
                else if (name == "alpha")
                    mAlpha = atof(value.c_str());
                else if (name == "beta")
                    mBeta = atof(value.c_str());
...
public:
    void TestAndVisualizeSphereParsed()
    {
        ParseParameters();
        // num_cells should be ~ pi*( 2^(1/3) / 3^(1/6) ) * (alpha-beta)^-2
        unsigned num_caspar_klug_h = mCasparKlugH;
        unsigned num_caspar_klug_k = mCasparKlugK;
        ...
        simulator.SetOutputDirectory(mSavePath);
        ...
        simulator.SetSamplingTimestepMultiple(180);
        simulator.SetDt(0.2);
        ...
        simulator.SetSamplingTimestepMultiple(450);
        simulator.SetDt(0.2);

        simulator.Solve();

        simulator.SetEndTime(1800.0);
        simulator.SetSamplingTimestepMultiple(1800);
        simulator.SetDt(0.2);
