...
public:
    void TestOutput()
    {
        double height = 2.0 / 3.0 / sqrt(3.0) * cbrt((9.0 / 2.0) * (9.0 / 2.0)) * cbrt(1.25 / 0.5 * 1.25 / 0.5) * 1.0;
        // double area = 1.0/height;
        //  length at which T1s become favorable: l^*=0.66/3^(2/3)/gamma^(1/3)
        double t1_length = 0.66 / cbrt(3 * 3 * (1.0 + 0.7) / 0.7);
        FiniteThicknessRandomizedSphereMeshGenerator generator(400, t1_length, 0.001, height, 10.0);
        ...
        TS_ASSERT_EQUALS(p_mesh->GetNumElements(), 400u);
        // comment
        ...
        simulator.SetOutputDirectory("Test3DSphere_Surface_Evolver_ApCon2.0");
        ...
        p_subforce->CreateSurfaceTensionParametersForCells(1.5, 0.5, 1.0, p_mesh); // apical, basal, lateral
        ...
        double in_rad = 10.0;
        double out_rad = 10.0 + height;
        double avg_cell_volume = 4.0 / 3.0 * M_PI * (out_rad * out_rad * out_rad - in_rad * in_rad * in_rad) / 400.0;
        ...
        p_subforce->SetSimulatedAnnealingParameters(0.0, 600.0, 10.0);
        p_subforce->SetSimulationInstance(&simulator);
