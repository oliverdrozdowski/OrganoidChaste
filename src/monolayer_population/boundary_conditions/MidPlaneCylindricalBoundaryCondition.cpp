...
MidPlaneCylindricalBoundaryCondition<ELEMENT_DIM, SPACE_DIM>::MidPlaneCylindricalBoundaryCondition(
...
{
    assert(radius > 0.0);
    mMidAxis = midAxis / norm_2(midAxis);
    mRadius = radius;
}

template <unsigned ELEMENT_DIM, unsigned SPACE_DIM>
const c_vector<double, SPACE_DIM>& MidPlaneCylindricalBoundaryCondition<ELEMENT_DIM, SPACE_DIM>::rGetMidPoint() const
{
    return mMidPoint;
}

template <unsigned ELEMENT_DIM, unsigned SPACE_DIM>
const c_vector<double, SPACE_DIM>& MidPlaneCylindricalBoundaryCondition<ELEMENT_DIM, SPACE_DIM>::rGetMidAxis() const
{
    return mMidAxis;
}

template <unsigned ELEMENT_DIM, unsigned SPACE_DIM>
double MidPlaneCylindricalBoundaryCondition<ELEMENT_DIM, SPACE_DIM>::GetRadius() const
{
    return mRadius;
}

template <unsigned ELEMENT_DIM, unsigned SPACE_DIM>
void MidPlaneCylindricalBoundaryCondition<ELEMENT_DIM, SPACE_DIM>::ImposeBoundaryCondition(
    const std::map<Node<SPACE_DIM>*, c_vector<double, SPACE_DIM> >& rOldLocations)
{
    ...
    {
        EXCEPTION("MidPlaneCylindricalBoundaryCondition requires a subclass of AbstractOffLatticeCellPopulation.");
    }
    ...
    if (SPACE_DIM != 1)
    {
        ...
        for (auto face_iter = r_mesh.GetFaceIteratorBegin(); face_iter != r_mesh.GetFaceIteratorEnd(); ++face_iter)
        {
            ...
            for (unsigned local_index = 0; local_index < num_nodes; local_index++)
            {
                ...
                distance_vector = distance_vector - inner_prod(distance_vector, mMidAxis) * mMidAxis;
    ...
    else
    {
        // DIM == 1
        NEVER_REACHED;
        // MidPlaneCylindricalBoundaryCondition::ImposeBoundaryCondition is not implemented in 1D
    }
}

template <unsigned ELEMENT_DIM, unsigned SPACE_DIM>
bool MidPlaneCylindricalBoundaryCondition<ELEMENT_DIM, SPACE_DIM>::VerifyBoundaryCondition()
{
    return true;
    if (SPACE_DIM == 1)
    {
        EXCEPTION("MidPlaneCylindricalBoundaryCondition is not implemented in 1D");
    }
    else
    {
        // NO CHECK!
...
template <>
void MidPlaneCylindricalBoundaryCondition<1, 1>::ImposeBoundaryCondition(const std::map<Node<1>*, c_vector<double, 1> >& rOldLocations)
{
    EXCEPTION("MidPlaneCylindricalBoundaryCondition only in 3D.");
}
/// \endcond Get Doxygen to ignore, since it's confused by these templates

/// \cond Get Doxygen to ignore, since it's confused by these templates
template <>
void MidPlaneCylindricalBoundaryCondition<1, 2>::ImposeBoundaryCondition(const std::map<Node<2>*, c_vector<double, 2> >& rOldLocations)
{
    EXCEPTION("MidPlaneCylindricalBoundaryCondition only in 3D.");
}
/// \endcond Get Doxygen to ignore, since it's confused by these templates
...
/// \cond Get Doxygen to ignore, since it's confused by these templates
template <>
void MidPlaneCylindricalBoundaryCondition<1, 3>::ImposeBoundaryCondition(const std::map<Node<3>*, c_vector<double, 3> >& rOldLocations)
{
    EXCEPTION("MidPlaneCylindricalBoundaryCondition only in 3D.");
}
/// \endcond Get Doxygen to ignore, since it's confused by these templates
...
/// \cond Get Doxygen to ignore, since it's confused by these templates
template <>
void MidPlaneCylindricalBoundaryCondition<2, 2>::ImposeBoundaryCondition(const std::map<Node<2>*, c_vector<double, 2> >& rOldLocations)
{
    EXCEPTION("MidPlaneCylindricalBoundaryCondition only in 3D.");
}
/// \endcond Get Doxygen to ignore, since it's confused by these templates
...
/// \cond Get Doxygen to ignore, since it's confused by these templates
template <>
void MidPlaneCylindricalBoundaryCondition<2, 3>::ImposeBoundaryCondition(const std::map<Node<3>*, c_vector<double, 3> >& rOldLocations)
{
    EXCEPTION("MidPlaneCylindricalBoundaryCondition only in 3D.");
}
/// \endcond Get Doxygen to ignore, since it's confused by these templates
...
/// \cond Get Doxygen to ignore, since it's confused by these templates
template <>
bool MidPlaneCylindricalBoundaryCondition<1, 1>::VerifyBoundaryCondition()
{
    EXCEPTION("MidPlaneCylindricalBoundaryCondition only in 3D.");
    return false;
}
...
/// \cond Get Doxygen to ignore, since it's confused by these templates
template <>
bool MidPlaneCylindricalBoundaryCondition<1, 2>::VerifyBoundaryCondition()
{
    EXCEPTION("MidPlaneCylindricalBoundaryCondition only in 3D.");
    return false;
}
...
/// \cond Get Doxygen to ignore, since it's confused by these templates
template <>
bool MidPlaneCylindricalBoundaryCondition<1, 3>::VerifyBoundaryCondition()
{
    EXCEPTION("MidPlaneCylindricalBoundaryCondition only in 3D.");
    return false;
}
...
/// \cond Get Doxygen to ignore, since it's confused by these templates
template <>
bool MidPlaneCylindricalBoundaryCondition<2, 2>::VerifyBoundaryCondition()
{
    EXCEPTION("MidPlaneCylindricalBoundaryCondition only in 3D.");
    return false;
}
...
/// \cond Get Doxygen to ignore, since it's confused by these templates
template <>
bool MidPlaneCylindricalBoundaryCondition<2, 3>::VerifyBoundaryCondition()
{
    EXCEPTION("MidPlaneCylindricalBoundaryCondition only in 3D.");
    return false;
}
...
template <unsigned ELEMENT_DIM, unsigned SPACE_DIM>
void MidPlaneCylindricalBoundaryCondition<ELEMENT_DIM, SPACE_DIM>::OutputCellPopulationBoundaryConditionParameters(out_stream& rParamsFile)
{
    ...
    *rParamsFile << "\t\t\t<MidAxis>";
    for (unsigned index = 0; index != SPACE_DIM - 1U; index++) // Note: inequality avoids testing index < 0U when DIM=1
    {
        *rParamsFile << mMidAxis[index] << ",";
    }
    *rParamsFile << mMidAxis[SPACE_DIM - 1] << "</MidAxis>\n";
...
template class MidPlaneCylindricalBoundaryCondition<1, 1>;
template class MidPlaneCylindricalBoundaryCondition<1, 2>;
template class MidPlaneCylindricalBoundaryCondition<2, 2>;
template class MidPlaneCylindricalBoundaryCondition<1, 3>;
template class MidPlaneCylindricalBoundaryCondition<2, 3>;
template class MidPlaneCylindricalBoundaryCondition<3, 3>;
